<script lang="ts">
  ...
  export async function initialize(micropolisSimulator_: MicropolisSimulator): Promise<void> {
    ...
    await tileRenderer.initialize(
      ...
      tileRenderer.setupSubRenderers(micropolisSimulator);
  ...
  export function onkeydown(event: KeyboardEvent): void {
    ...
    if ((key >= "0") && (key <= "9")) { // digits
      ...
      const mapStartAddress = micropolisSimulator.micropolis.getMapAddress() / 2;
      const mapEndAddress = mapStartAddress + micropolisSimulator.micropolis.getMapSize() / 2;
      console.log({mapStartAddress,mapEndAddress});
      console.log(micropolisSimulator);
    }
    else switch (key) {
